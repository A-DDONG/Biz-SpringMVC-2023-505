...
@Service
public class AddServiceImplV1 implements AddrService {
	
	/*
	 * 의존성 주입(Dependency Inject: DI)
	 * 객체를 선언하고, 클래스의 생성자를 통해 객체를 초기화하는
	 * java 의 전통적인 객체생성 코드와 대비되는 Spring Framework Project 의 독특한 기법
	 * 객체를 선언만 하고, 의존성 주입 절차를 수행하면
	 * Spring Container 에 의해 이미 생성되어 준비된 객체를 주입해주는 기능
	 * 
	 * 의존성 주입하는 방법은 필드주입, 생성자주입, setter 주
	 */
	/*
	 * 필드 의존성 주입(Field Dependency Inject(DI))
	 * 
	 * 		필드(클래스영역)에 객체를 선언하고 @Autowired 를 부착하는 방법
	 * 		Spring Project 의 전통적인 코딩에서 가장 많이 사용하던 방법
	 * 		이 방법이 메모리 관리 측면에서 문제가 있다는 보고가 있으면서 사용이 점차 줄어들고 있다
	 * 		이 방법은 메모리 누수를 줄이기 위하여 객체를 private 으로 선언할것을 권장하는 방법이다
	 ...
	 * 		이 방식의 특징은 한번 주입이 되면, 코드가 작동되는 중에 또다시 주입을 할 수 없다. 객체 보호가 완전하다
	 * 		객체의 순환참조가 없다
	 ...
	 * 		코드가 다소 번잡해진다
	 * 		실수로 생성자 메서드를 만들지 않았을 경우 NullPointer Exception 이 발생한다
	 * 
	 */
