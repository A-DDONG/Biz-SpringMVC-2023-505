...
public class UserController {
	 ...
	 * error 매개변서 값은 그대로 model 에 담아서 login.jsp 로 보낸다
	 */
	...
	/*
		...
		Spring 에서는 DespatcherServlet 이 httpSession 객체를 생성하여 method 의 매개변수로 전달해준다
		...
		model 에 데이터를 담는것과 차이는 model 데이터는 한번 view 와 rendering 을 수행한 후 소멸
										  session 데이터는 특별한 조건하에서 계속 유지된다
										  또한 session 데이터가 필요 없을 경우 이 데이터를 강제 소멸시켜야 한다
	 */
	...
	public String login(UserDto userDto, HttpSession httpSession) {
		...
		if(result.equals("OK")) {
			/*
			 * add() : 무한히 데이터를 추가할수있다 라는 의미
			 * set() : 제한적으로 데이터를 추가하는 것이 좋다라는 의미
			 * httpSession 에 set 한 Attribute 는 
			 * model 에 add 한 Attribute 와 view 에서 사용하는 것은 거의 동일하다
			 */
