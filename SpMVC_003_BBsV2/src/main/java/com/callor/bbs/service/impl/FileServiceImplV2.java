...
public class FileServiceImplV2 extends FileServiceImplV1{
	...
	// fileup-context.xml 에 선언된 2개의 String bean 값을 사용할수 있도록 주입해달라
	...
	public FileServiceImplV2(ResourceLoader resource, String windowsPath, String winPath, String macHome, String macPath) {
		/*
		 * super()
		 * 상속받은 클래스의 생성자에게 무언가 전달할때 사용하는 methmod
		 * 일종의 전달자
		 * V1 클래스의 생성자에게 resource 객체를 전달하여 final 로 선언된
		 * 2개의 변수(객체)를 초기화 해달라고 요청		
		 *  */
		super(resource);
...
//		}
		...
		// Java 에서 제공하는 UUID 를 생성하는 코드
		String strUUID = UUID.randomUUID().toString();
		// image.jpg0000-0000-0000
		
		// UUID 키값을 파일이름 앞에 부착하여 새로운 이름으로 변경
	...
	public List<FileDto> filesUp(MultipartHttpServletRequest files) throws Exception {
		...
		/*
		 * 멀티파일을 각 파일로 분리하여 fileUp() 에게 파일을 업로드하도록 요청하고
