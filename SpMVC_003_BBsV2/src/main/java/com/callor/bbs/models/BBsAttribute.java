...
/*
 * 날짜클래스
 * Java 에서는 전통적으로 Date, Calendar 와 같은 날짜(시간)관련 클래스를 가지고 있다
 * 이 클래스들은 날짜값에서 여러가지 이슈들이 많이 발견되었다
 * 또한 Java 가 업그레이드 되는 동안, 날짜와 관련된 여러가지 문제를 해결하지 못하는 단점이 있었다
 ...
 * Java 와 DB Server 간에 데이터 교환에 문제를 일으키기도 한다
 * 보통 DB 와 연동을 할때 날짜 형식의 데이터를 사용하지 않고 문자열로 변환하여 사용한다
 */
