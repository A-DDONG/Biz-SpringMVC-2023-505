document.addEventListener("DOMContentLoaded", () => {
  ...
  const bnameSearch = async (e) => {
    ...
    // result 배열(List) 전체를 반복 순회하면서 각각의 요소를 book 에 담아 내부의 함수로 전달하라
    result.forEach((book) => {
      console.table(book);
  ...
  /*
  키보드 관련 이벤트
  keydown : 키가 눌리는 순간, Enter, Esc 등 특수키값을 Capture 할 수 있다
  keypress : keydown 과 거의 유사
  keyup : 키보드에서 손이 떼어지는 순간, 숫자, 영문자 등 키보드에 표시된 문자들 Capture
  */
