 ...
 * Dto : CRUD 를 위한 Data Class 이다 -> getter, setter, 실제 data 가 저장되는 Table 과 1:1 로 칼럼이 매칭
 * VO : SELECT 를 위한 Data Class 이다 -> getter 만 있는 상태, 한 번 저장된 데이터를 변경할 수 없다. 읽기전용
 * 											JOIN  되는 SELECT 문의 결과를 DB로부터 가져와서 데이터를 저장하는 클래스	
 */											
@Getter
@Setter
@ToString
