 ...
 * HttpSession : Servlet 차원에서 Http Protocol 상에 구현하는 Session 로그인과 관련된 Session 은 이 클래스를 사용한다
 * SessionAttributes : Spring Framework 차원에서 구현하는 Session, 일반적인 객체등을 Session 에 저장하여 다양한 기능을 구현하는 목적
 * Session Server 의 메모리 영역을 일정부분 차지한다
 * 접속자가 많아지면 접속자만큼 메모리 소모가 일어난다. 때문에 Session 은 꼭 필요한 요소에서만 제한적으로 사용하는 것이 좋다
...
public class RentController {
	...
	public String work_member(@ModelAttribute("RENT_WORK") RentBookVO rentBookVO) {
		 ...
		 * view 로 전달을 해야한다. 하지만 @ModelAttribute 를 사용하면 자동으로 Model 객체에
	...
	/*
	 * method 에 @ModelAttribute("객체") 가 부착되면 어디선가 객체가 필요로 할때 자동으로 method 가 실행되고
	 * 객체가 생성되어 주입된다 	
	 */
	...
	public RentBookVO newRentBook() {
		...
		// Java 1.8 이후에서는 Date, Calendar 클래스 보다는 Local** 클래스를 적극 사용하기를 권장
		// Java 는 1.8 을 기준으로 이전과 이후를 다른 세대로 구분한다. 1.7 이하는 1.x 로 표기 1.8 부터는 8, 9, 10 등으로 표기한다
